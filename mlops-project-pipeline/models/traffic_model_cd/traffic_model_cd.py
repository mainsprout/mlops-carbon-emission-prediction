...
os.environ['FEATURE_STORE_URL'] = f"mysql+pymysql://root:root@localhost:3307/mlops_project_meta"
...
service1 = "traffic_model_api_1"
service2 = "traffic_model_api_2"
...
def _get_api_services():
    api_services = []
    command = f"cd {airflow_dags_path}/models/traffic_model_cd && " \
              f"export ENCODERS_PATH={api_env['ENCODERS_PATH']} && " \
              f"export MODEL_PATH={api_env['MODEL_PATH']} && " \
...
def get_next_branch(ti):
    # XCom에서 데이터 가져오기
    ct_model_version = ti.xcom_pull(key=xcom_model_version_key)
    training_cutoff_date = ti.xcom_pull(key=xcom_training_cutoff_date_key)

    print("api_env = ", api_env)
    api_env["MODEL_NAME"] = model_name
    api_env["MODEL_VERSION"] = ct_model_version
    api_env["ENCODERS_PATH"] = f"data_preparation/{model_name}/ct/{training_cutoff_date}/encoders"
    api_env["MODEL_PATH"] = f"model/{model_name}/{ct_model_version}/{training_cutoff_date}"
    print("after_api_env = ", api_env)
...
with DAG(dag_id="traffic_model_cd",
         default_args={
             "owner": "mlops.study",
